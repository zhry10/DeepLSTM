...
with tf.device('/device:GPU:1'):
    ...
    for e in range(epochs):
        ...
        if test_loss[e] < best_loss:
            ...
            model.save(dataDir + 'results/Bouc-Wen (LSTM-s)/my_best_model.h5')
...
# Load the best model
model_best = load_model(dataDir + 'results/Bouc-Wen (LSTM-s)/my_best_model.h5')
...
# Save scaler
joblib.dump(scaler_X, dataDir+'results/Bouc-Wen (LSTM-s)/scaler_X.save')
joblib.dump(scaler_y, dataDir+'results/Bouc-Wen (LSTM-s)/scaler_y.save')
# And now to load...
# scaler_X = joblib.load(dataDir+'results/Bouc-Wen (LSTM-s)/scaler_X.save')
# scaler_y = joblib.load(dataDir+'results/Bouc-Wen (LSTM-s)/scaler_y.save')

scipy.io.savemat(dataDir+'results/Bouc-Wen (LSTM-s)/results_BW.mat',
