...
mat = scipy.io.loadmat(dataDir+'data/data_MRFDBF.mat')
...
# Load the best model
model_best = load_model(dataDir+'results/MRF-DBF/my_best_model.h5')
...
for sample in range(len(y_test)):
    plt.figure()
    plt.plot(y_test_orig[sample][0::windowsize, 0], label='True')
...
for sample in range(len(y_pred_ref)):
    plt.figure()
    plt.plot(y_pred_ref_orig[sample][0::windowsize, 0], label='True')
    plt.plot(y_pure_preds[sample][:, 0], label='Predict')
    plt.title('Prediction')
    plt.legend()

# Save scaler
joblib.dump(scaler_X, dataDir+'results/MRF-DBF/scaler_X.save')
joblib.dump(scaler_y, dataDir+'results/MRF-DBF/scaler_y.save')
# And now to load...
# scaler_X = joblib.load(dataDir+'results/MRF-DBF/scaler_X.save')
# scaler_y = joblib.load(dataDir+'results/MRF-DBF/scaler_y.save')

scipy.io.savemat(dataDir+'results/MRF-DBF/results_MRFDBF.mat',
